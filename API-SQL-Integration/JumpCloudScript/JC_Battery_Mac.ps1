...
# Save info
$response = Get-JCSystemInsights -Table:('Battery')
#$response | Export-Csv -Path "C:\JCBattery.csv" -NoTypeInformation

# Convert to JSON
$responseObj = ConvertTo-Json $response

$add1 = "INSERT INTO JSON_JumpCloud ( JSON_Date, DATASET, JSON_Data ) VALUES ('" + $RunTime + "', 'BATTERY', '" + $responseObj + "')"
